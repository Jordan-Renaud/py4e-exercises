...
try:
    ...
    for line in fhand:
        ...
        count += 1
        if count > 8:
            print("For briefness this is enough printed out")
            break
except:
...
the line to extract the floating-point number on the line. Count these lines a
nd then compute the total of the spam confidence values from these lines.
...
try:
    ...
    #starts going through the lines in the file
    for line in fhand:
        #finds lines with the correct line start
        if line.startswith("X-DSPAM-Confidence: "):
            index_of_float = line.find("0.")
            float_count += float(line[index_of_float:])

            line_count += 1
    print(float_count / line_count)
except:
    print("Could not open file:", file_name)
